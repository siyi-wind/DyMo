...
from models.utils.MVAE.divergence_measures.kl_div import calc_kl_divergence_lb_gauss_mixture
from models.utils.MVAE.divergence_measures.kl_div import calc_kl_divergence_ub_gauss_mixture
from models.utils.MVAE.divergence_measures.kl_div import calc_entropy_gauss

from models.utils.MVAE.utils import reweight_weights
...
def calc_alphaJSD_modalities_mixture(m1_mu, m1_logvar, m2_mu, m2_logvar, flags):
    ...
    entropies_mixture = entropies_mixture.to(flags.device);
...
def calc_modality_divergence(m1_mu, m1_logvar, m2_mu, m2_logvar, flags):
    if flags.modality_poe:
        ...
        klds = klds.to(flags.device);
        klds_modonly = klds_modonly.to(flags.device);
        uniform_mu = uniform_mu.to(flags.device);
        uniform_logvar = uniform_logvar.to(flags.device);
