...
from models.utils.MVAE.divergence_measures.mm_div import calc_alphaJSD_modalities
from models.utils.MVAE.divergence_measures.mm_div import calc_group_divergence_moe
from models.utils.MVAE.divergence_measures.mm_div import poe
...
class BaseMMVae(ABC, nn.Module):
    ...
    def forward(self, input_batch):
        self.flags.device = str(input_batch[list(input_batch.keys())[0]].device);
    ...
    def cond_generation(self, latent_distributions, num_samples=None):
        # generate from conditional distributions
        if num_samples is None:
            num_samples = self.flags.batch_size;
