...
class DyMo(nn.Module):
    ...
    def calculate_reward(self, u: int, x: Dict, mask: torch.Tensor, loc: torch.Tensor, y: torch.Tensor, visualize: bool = False):
        '''
        ...
        '''
        # TODO different prototypes
        prototypes = self.subset_gaussian['overall_prototypes'].to(mask.device)  # (num_subsets, num_classes, dim)
        # prototypes = self.model.prototypes.clone().detach()  # (num_subsets, num_classes, dim)
        ...
        if self.quality_metric_name == 'prototype_distance':
            ...
            if self.distance_metric == 'cosine_similarity':
                ...
                elif self.dataset_name in set(['CAD', 'Infarction']):
                    G = torch.where(gaussian_probs_u < gaussian_probs, (gaussian_probs_u / (gaussian_probs + 1e-9)), G)
            elif self.distance_metric == 'squared_euclidean':
                G = torch.where(gaussian_probs_u < gaussian_probs, (gaussian_probs_u / (gaussian_probs + 1e-9)), G)
            else:
        ...
        if visualize:
            return R, quality, quality_u, gaussian_probs, gaussian_probs_u
        else:
            return R
    ...
    def forward(self, x: Dict, mask: torch.Tensor, y: torch.Tensor, visualize: bool = False):
        ...
        with torch.no_grad():
            ...
            if self.imputer_name == 'TIP':
                ...
                pass  # Input has already been imputed
        ...
        Q_history = torch.zeros(T, B, M).to(mask.device)  # quality history
        Q_u_history = torch.zeros(T, B, M).to(mask.device)  # quality history for updated mask
        G_history = torch.zeros(T, B, M).to(mask.device)  # Gaussian scores history
        G_u_history = torch.zeros(T, B, M).to(mask.device)  # Gaussian scores history for updated mask
        ...
        for t in range(T):
            ...
            # for visualization
            Q = torch.zeros(B, M).to(mask.device)  # quality history
            Q_u = torch.zeros(B, M).to(mask.device)  # quality history for updated mask
            G = torch.zeros(B, M).to(mask.device)  # Gaussian scores history
            G_u = torch.zeros(B, M).to(mask.device)  # Gaussian scores history for updated mask
            for u in range(self.num_modalities):
                ...
                if loc.sum() == 0:
                    continue
                if visualize:
                    R[loc, u], Q[loc, u], Q_u[loc, u], G[loc, u], G_u[loc, u] = self.calculate_reward(u, x, mask, loc, y, visualize=True)
                else:
            ...
            Q_history[t, ...] = Q.clone().detach()
            Q_u_history[t, ...] = Q_u.clone().detach()
            G_history[t, ...] = G.clone().detach()
            G_u_history[t, ...] = G_u.clone().detach()
        ...
        if visualize:
            return out, {'R_history': R_history, 'Q_history': Q_history, 'Q_u_history': Q_u_history, 
                         'G_history': G_history, 'G_u_history': G_u_history, 
                         'mask_history': mask_history, 'action_history': action_history}
        else:
            return out
...
if __name__ == "__main__":
    ...
    args = {'DVM':{"transformer_dim": 256, "transformer_heads": 8, "transformer_layers": 2, "transformer_drop": 0.0, 
                         "num_patches_list": [16,17], "num_masks": 0, 'imputer_name': 'TIP', "distance_metric": 'cosine_similarity',
                         'imputer_checkpoint': '/bigdata/siyi/data/result/D20/MaskAttn_ran00spec05_dvm_0104_0938/checkpoint_last_epoch_499.ckpt',
                         'transformer_name': 'DynamicTransformer_singleCLS',
                         'transformer_checkpoint': '/home/siyi/project/mm/result/Dynamic_project/DV4/none_DVM_DynamicTransformer_singleCLS_0519_184916/downstream/checkpoint_best_acc.ckpt',
                         'quality_metric_name': 'prototype_distance', 'projection_dim': 128,}, 
                         'DATA_field_lengths_tabular': '/bigdata/siyi/data/DVM/features/tabular_lengths_all_views_physical_reordered.pt',
                         'checkpoint': None, 'num_classes': 286, 'missing_tabular': False,
                         'dataset_name': 'DVM', 'batch_size':2, 'logdir':None,
                "num_modalities": 2, 'alphabet': alphabet, 'modality_names': ['img', 'tabular'],'len_sequence': 256}
