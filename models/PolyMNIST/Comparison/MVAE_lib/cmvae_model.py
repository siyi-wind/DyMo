...
from multivae.models.cmvae.cmvae_config import CMVAEConfig
...
class CMVAE_Batch(BaseMultiVAE):
    """
    Adaptation of CMVAE to batch feature extraction.
    ...
    def _iwae_looser(self, lws, n_mods_sample):
        ...
        lws = lws.sum(0) / n_mods_sample.to(lws.device)  # n_batch
    ...
    def encode_batch(
        self,
        x: Dict, mask: torch.Tensor,
        **kwargs,
    ):
        """
        Self-defined function to encode a batch of data with different per-sample cond_mod
        Note: mask == True indicates unavailable modality
        """
        out = []
        for i, name in enumerate(self.modality_names):
            assert f'm{i}' == name, "modality names do not match!"
            tmp = self.encoders[name](x[name]).embedding
            out.append(tmp)
        out = torch.stack(out, dim=1)  # (B, M, D)

        # Randomly choose one of the available modalities for each sample
        B, M = mask.shape
        rand = torch.rand_like(mask, dtype=torch.float, device=mask.device)  # (B, M)
        rand = rand + mask*(-1e9) # set unavailable modalities to very small value
        random_mod_idx = torch.argmax(rand, dim=1)  # (B,)
        # Guarantee that mask[id] == False
        assert torch.all(mask[torch.arange(B), random_mod_idx] == False), "Selected modality is unavailable!"
        mu = out[torch.arange(B), random_mod_idx, :]  # (B, D)
        return ModelOutput(z=mu, one_latent_space=True)
