...
class OnlineMAE(nn.Module):
    ...
    def __init__(self, args):
        ...
        for i in range(args.num_modalities):
            ...
            self.m_projection[f'm{i}'] = nn.Linear(512, args[args.dataset_name].transformer_dim)
    ...
    def forward_recon(self, x: Dict, mask: torch.Tensor, random_masking: bool = True):
        ...
        for i, name in enumerate(self.modality_names):
            ...
            tmp = self.m_projection[name](tmp)  # (B, 1, C)
    ...
    def forward_train(self, x: Dict, mask: torch.Tensor, y: torch.Tensor = None):
        ...
        # print(torch.softmax(out.detach(), dim=-1)[0])
...
if __name__ == "__main__":
    args = {'PolyMNIST':{"transformer_dim": 128, "transformer_heads": 4, "transformer_layers": 2, "transformer_drop": 0.0, "num_patches_list": [1, 1, 1, 1, 1], 
                ...
                "num_modalities": 5, "checkpoint": None, 'modality_names': ["m0", "m1", "m2", "m3", "m4"],
                "num_classes": 10}
    ...
    mask = torch.tensor([[False, True, True, False, False], [False, False, False, True, True]])  #
