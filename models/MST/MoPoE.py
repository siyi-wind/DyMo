...
from models.utils.MVAE.Modality import MNIST, SVHN, Text
from itertools import chain, combinations
from models.utils.MVAE.divergence_measures.kl_div import calc_kl_divergence
from models.utils.MVAE import utils
from models.MST.networks.MoPoE_ConvNetworksImgMNIST import EncoderImg, DecoderImg
from models.MST.networks.MoPoE_ConvNetworksImgSVHN import EncoderSVHN, DecoderSVHN
from models.MST.networks.MoPoE_ConvNetworksTextMNIST import EncoderText, DecoderText
...
class MoPoE(nn.Module):
    ...
    def __init__(self, args):
        ...
        flags = {'modality_moe': False, 'modality_jsd': False, 'joint_elbo': True, 'modality_poe': False,
                 ...
                 'device': None, 'class_dim': 20, 'style_dim': 0, 'factorized_representation': False,
                 'dir_checkpoints':args.logdir, 'beta_style': 1.0, 'beta_content': 1.0, 'beta': 5.0, 
                 'beta_m1_style': 1.0, 'beta_m2_style': 1.0, 'beta_m3_style': 1.0,  'num_features': 71, 'num_hidden_layers':1,
                 'dim': 64}
    ...
    def set_modalities(self,):
        mod1 = MNIST("mnist", EncoderImg(self.flags), DecoderImg(self.flags), 10, 0, 'laplace')
        mod2 = SVHN("svhn", EncoderSVHN(self.flags), DecoderSVHN(self.flags), 10, 0, 'laplace', self.plot_img_size)
        mod3 = Text("text", EncoderText(self.flags), DecoderText(self.flags), 10, 0, 'categorical',
                    self.len_sequence, self.alphabet, self.plot_img_size, self.font)
        mods_dict = {m.name: m for m in [mod1, mod2, mod3]}
        return mods_dict
    ...
    def set_subsets(self):
        ...
        subsets_list = chain.from_iterable(combinations(xs, n) for n in
                                          range(len(xs)+1))
        subsets = dict();
        mod_names_dict = {'mnist': 0, 'svhn': 1, 'text': 2}
        self.subset2name = {}
...
if __name__ == "__main__":
    ...
    args = {'MST':{"transformer_dim": 32, "transformer_heads": 2, "transformer_layers": 2, "transformer_drop": 0.0, "num_patches_list": [1, 1, 1], 
                         "num_masks": 0}, 'dataset_name': 'MST',
                "num_modalities": 3, "checkpoint": "/home/siyi/project/mm/result/Dynamic_project/MS6/MNIST_SVHN_Text/joint_elbo/non_factorized/laplace_laplace_categorical/SVHN_MNIST_text_2025_05_05_14_13_16_648881/checkpoints/0199/mm_vae", 'logdir': None, 'batch_size': 2,
                "downstream_strategy": "frozen", 'alphabet': alphabet, 'modality_names': ['mnist', 'svhn', 'text'], "len_sequence": 8, }
