...
def load_datasets(hparams):
    if hparams.dataset_name == 'PolyMNIST':
        ...
        train_dataset = PolyMNISTDataset(
            ...
            target_transform=None, low=hparams.low, return_idx=hparams.return_idx)
        ...
        train_dataset = SVHNMNIST(flags, alphabet, train='train', missing_path=hparams.missing_train, transform=get_transforms(hparams.image_size, hparams.target, 'train'), return_idx=hparams.return_idx)
        ...
        with open(alphabet_path) as alphabet_file:
            alphabet = str(''.join(json.load(alphabet_file)))
        train_dataset = CelebaDataset(flags, alphabet, missing_path=hparams.missing_train, partition=0, transform=get_transforms(hparams.image_size, hparams.target, 'train'), return_idx=hparams.return_idx)
        ...
        train_dataset = ImagingAndTabularDataset(
                    ...
                    missing_path=hparams.missing_train, return_idx=hparams.return_idx)
...
def downstream(hparams, wandb_run):
    # seed everything
    ...
    sub_logdir = join(hparams.logdir, 'downstream')
    os.mkdir(sub_logdir)
    if hparams.model_name == 'DynamicTransformer':
        from experiments.DownstreamAugmentationExperiment import DownstreamAugExperiment
        experiment = DownstreamAugExperiment(hparams, wandb_run, sub_logdir)
    else:
        from experiments.DownstreamExperiment import DownstreamExperiment
        experiment = DownstreamExperiment(hparams, wandb_run, sub_logdir)
    ...
    test_results = experiment.test(test_loader, join(sub_logdir, f'checkpoint_best_{hparams.eval_metric}.ckpt'))
